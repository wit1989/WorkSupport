...
def get_namelist(event):
    ...
    for i in wchat_getlist_trv.selection():
        ...
        sub_treeview.pack(side=tk.LEFT, pady=2)
        for i in range(len(column_list)):
            sub_treeview.heading(column_list[i], text=column_ch[i])
        # sub_treeview.heading('number', text='序号')  # 设置字段的显示名称
        # sub_treeview.heading('nickname', text='微信名称')
        # sub_treeview.heading('displayname', text='群昵称')
        # sub_treeview.heading('phonenum', text='手机号')
        # sub_treeview.heading('sex', text='性别')
        # sub_treeview.heading('location', text='城市')
...
def to_excel():
    ...
    last_name = file_name('"%s"群成员名单.xlsx' % group_name)
    wb.save(last_name)
    tkinter.messagebox.showinfo("提示", "输出完成，请查看《" + last_name + "》")
...
datamenu.add_command(label='DBF4', command=non_standard)
datamenu.add_command(label="in('')", command=non_standard)
...
flist = tk.StringVar()
# flist.set((''))
merge_excel_listbox = tk.Listbox(merge_excel_frm0, listvariable=flist).pack(fill='both', expand=1, padx=3)
