...
data_comparison_frm4 = tk.Frame(data_comparison_frm0, pady=4)
data_comparison_frm4.pack(fill='both', expand=1)
data_comparison_lf = tk.Frame(data_comparison_frm4, pady=2)
data_comparison_lf.pack(fill='both', expand=1)
data_comparison_tv = ttk.Treeview(data_comparison_lf, show="headings", columns=listhead)
for col in listhead:
...
merge_excel_frm1.pack(fill='both', expand=1, pady=10)
merge_excel_text = tk.Label(merge_excel_frm1,
                            text="\n提示:\n1、将需要合并的文件放在同一文件夹下；\n2、数据表放在sheet1；\n3、文件表头行数要相等。\n",
                            font=myfont, justify='left').pack()
fdir = tk.StringVar()
merge_excel_ent = tk.Entry(merge_excel_frm1, width=40, font=myfont, textvariable=fdir).pack(fill=tk.X, expand=1, side=tk.LEFT)
...
about_this_lb = tk.Label(about_this_frm1, text="版本号：0.14\n\n作者：陈博文", font=myfont).pack(fill='both', expand=1)
